...
function SidebarContent({ isCollapsed }) {
  return (
    <ScrollArea className="flex flex-col flex-grow">
      <div className={cn("flex h-14 items-center border-b px-3", isCollapsed ? "justify-center" : "px-4")}>
        {isCollapsed ? <TrendingUp className="h-6 w-6" /> : <h1 className="text-xl font-semibold">Stock Predictor</h1>}
      </div>
